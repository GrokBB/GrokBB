<?php
// https://github.com/symfony/http-foundation/blob/master/Session/Storage/Handler/PdoSessionHandler.php
// Version: 023606d on Jul 7, 2015 (downloaded Jan 22, 2016)
// Updates: Changed the namespace and class names and added query logging, i.e. $GLOBALS['db']->log[]
...
class Session implements \SessionHandlerInterface
{
    ...
    public function createTable()
    {
        ...
        switch ($this->driver) {
            case 'mysql':
                ...
                $sql = "CREATE TABLE $this->table ($this->idCol VARBINARY(128) NOT NULL PRIMARY KEY, $this->dataCol MEDIUMBLOB NOT NULL, $this->lifetimeCol MEDIUMINT NOT NULL, $this->timeCol INTEGER UNSIGNED NOT NULL) COLLATE utf8_bin, ENGINE = InnoDB";
                break;
            case 'sqlite':
    ...
    public function destroy($sessionId)
    {
        ...
        try {
            ...
            $GLOBALS['db']->log[] = $stmt->queryString;
            $stmt->execute();
        } catch (\PDOException $e) {
            $this->rollback();
    ...
    public function write($sessionId, $data)
    {
        ...
        try {
            ...
            if (null !== $mergeSql) {
                ...
                $GLOBALS['db']->log[] = $mergeStmt->queryString;
                $mergeStmt->execute();
            ...
            $GLOBALS['db']->log[] = $updateStmt->queryString;
            $updateStmt->execute();
            ...
            if (!$updateStmt->rowCount()) {
                try {
                    ...
                    $GLOBALS['db']->log[] = $insertStmt->queryString;
                    $insertStmt->execute();
                } catch (\PDOException $e) {
    ...
    private function beginTransaction()
    {
        if (!$this->inTransaction) {
            if ('sqlite' === $this->driver) {
                $GLOBALS['db']->log[] = 'BEGIN IMMEDIATE TRANSACTION';
                $this->pdo->exec('BEGIN IMMEDIATE TRANSACTION');
            } else {
                if ('mysql' === $this->driver) {
                    $GLOBALS['db']->log[] = 'SET TRANSACTION ISOLATION LEVEL READ COMMITTED';
                    $this->pdo->exec('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
                }
                $GLOBALS['db']->log[] = 'START TRANSACTION';
    ...
    private function doRead($sessionId)
    {
        ...
        $GLOBALS['db']->log[] = $selectStmt->queryString;
        $selectStmt->execute();
    ...
    private function doAdvisoryLock($sessionId)
    {
        switch ($this->driver) {
            case 'mysql':
                ...
                $GLOBALS['db']->log[] = $releaseStmt->queryString;
